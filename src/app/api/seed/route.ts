...
const PRODUK_POOL = [
  {
    ...
    category: "Sembako",
    variants: [
      { name: "5kg", unit: "karung" },
      { name: "10kg", unit: "karung" },
      { name: "25kg", unit: "karung" },
    ],
  },
  {
    name: "Gula Pasir",
  ...
  },
  {
    ...
    category: "Kebersihan",
    variants: [
      { name: "Batang 80g", unit: "pcs" },
      { name: "Cair 250ml", unit: "botol" },
    ],
  },
  {
    name: "Deterjen Rinso",
    category: "Kebersihan",
    variants: [
      { name: "Sachet 35g", unit: "pcs" },
      { name: "800g", unit: "bungkus" },
    ],
  },
  {
    ...
    category: "Minuman",
    variants: [
      { name: "230ml", unit: "botol" },
      { name: "1 Krat (24btl)", unit: "krat" },
    ],
  },
  {
    name: "Aqua",
    ...
    variants: [
      { name: "240ml", unit: "pcs" },
      { name: "600ml", unit: "botol" },
      { name: "1500ml", unit: "botol" },
    ],
  },
  {
    name: "Kopi Kapal Api",
    category: "Minuman",
    variants: [
      { name: "Sachet 25g", unit: "pcs" },
      { name: "165g", unit: "bungkus" },
    ],
  },
  {
    name: "Susu SGM",
    category: "Susu",
    variants: [
      { name: "400g", unit: "kaleng" },
      { name: "900g", unit: "kaleng" },
    ],
  },
  {
    ...
    category: "Rokok",
    variants: [
      { name: "Batang", unit: "pcs" },
      { name: "Bungkus", unit: "bungkus" },
      { name: "1 Slop", unit: "slop" },
    ],
  },
  {
    name: "Garam Kapal",
    category: "Bumbu",
    variants: [
      { name: "250g", unit: "bungkus" },
      { name: "1kg", unit: "bungkus" },
    ],
  },
  {
  ...
  },
  {
    ...
    category: "Bumbu",
    variants: [
      { name: "Sachet 8g", unit: "pcs" },
      { name: "200g", unit: "bungkus" },
    ],
  },
  {
    name: "Royco Sapi",
...
export async function POST(req: Request) {
  ...
  if (userCount > 0 && !body.force) {
    return NextResponse.json(
      { error: "Database sudah berisi data. Gunakan --force-seed untuk menimpa." },
      { status: 409 },
    )
  }
  ...
  await Promise.all([
    ...DEFAULT_CATEGORIES.map((name) =>
      prisma.categoryOption.upsert({ where: { name }, update: {}, create: { name } }),
    ),
    ...DEFAULT_UNITS.map((name) =>
      prisma.unitOption.upsert({ where: { name }, update: {}, create: { name } }),
    ),
    prisma.customer
      .findFirst({ where: { name: "UMUM" } })
      .then((existing) => existing ?? prisma.customer.create({ data: { name: "UMUM" } })),
  ])
  ...
  const paymentMethods = await Promise.all(
    pmNames.map((name) =>
      prisma.paymentMethod.upsert({ where: { name }, update: {}, create: { name } }),
    ),
  )
  ...
  for (let i = 0; i < 30; i++) {
    ...
    customers.push(
      await prisma.customer.create({
        data: { name: faker.person.fullName(), phone, address: faker.location.streetAddress() },
      }),
    )
  }
  ...
  for (let i = 0; i < 200; i++) {
    ...
    const selected = faker.helpers.arrayElements(allVariants, faker.number.int({ min: 2, max: 5 }))
    ...
    const paymentAmount =
      Math.ceil(subtotal / 1000) * 1000 + faker.helpers.arrayElement([0, 5000, 10000, 20000])
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const selected = faker.helpers.arrayElements(allVariants, faker.number.int({ min: 1, max: 5 }))
  ...
  return NextResponse.json({
    ok: true,
    message: "Seed selesai. Login: admin@kasir.com / password123",
  })
}
